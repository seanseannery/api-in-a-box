...
/** REST controller implementing the book CRUD API. */
@RestController
public class BookController implements BookApi {
  ...
  public ResponseEntity<BookProto.Book> createBook(
      BookProto.Book bookProto, @AuthenticationPrincipal Jwt jwt) {
  ...
  public ResponseEntity<BookProto.BookList> getAllBooks(@AuthenticationPrincipal Jwt jwt) {
    ...
    books.stream().map(bookMapper::toProto).forEach(builder::addBooks);
    return ResponseEntity.ok(builder.build());
  }

  @Override
  public ResponseEntity<BookProto.Book> updateBook(
      String id, BookProto.Book bookProto, @AuthenticationPrincipal Jwt jwt) {
    Book book = bookMapper.toModel(bookProto);
