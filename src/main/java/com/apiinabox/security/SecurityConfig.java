...
/** Configures the Spring Security filter chain based on the authorization toggle. */
@Configuration
@EnableWebSecurity
public class SecurityConfig {
  ...
  /** Builds the security filter chain, enabling JWT auth when authorization is enabled. */
  @Bean
  public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
    if (authorizationConfig.isAuthorizationEnabled()) {
      http.authorizeHttpRequests(
              auth ->
                  auth.requestMatchers("/api/books/**").hasRole("USER").anyRequest().permitAll())
  ...
  /** Configures the JWT decoder using the OAuth2 provider's JWK Set URI. */
  @Bean
